...
function save_handler() {
	...
	if(image.boxes.length == 0)
		alert("Please draw some boxes around the cars")
		...
		var string_prefix = "http://www.vishalanand.net/CV_UCSD_images/"
		id = id.substring(string_prefix.length);
		console.log(id);
		...
		xmlhttp.open("GET", "hpu1/save/" + response, true)
		xmlhttp.send()
...
function load_image() {
	...
	xmlhttp.open("GET", "hpu1/get", false)
	xmlhttp.send()

	//var random_num = Math.floor(Math.random() * 170);
	//this.src = "/images/test-" + random_num + ".jpg";
	this.src = xmlhttp.response;
